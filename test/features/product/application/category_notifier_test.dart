  ...
  group('CategoryListNotifier', () {
    ...
    setUp(() {
      ...
      container = ProviderContainer(
        overrides: [categoryServiceProvider.overrideWithValue(mockService)],
      );
    });
    ...
    test('loadCategories 成功加载', () async {
      ...
      ).thenAnswer((_) async => [const CategoryModel(id: 1, name: '食品')]);
    ...
    test('addCategory 调用后会刷新列表', () async {
      ...
      when(
        () => mockService.addCategory(name: '饮料', parentId: null),
    ...
    test('updateCategory 失败时设置 error', () async {
      when(
        () => mockService.updateCategory(id: 1, name: '新', parentId: null),
      ).thenThrow(Exception('更新失败'));

      final notifier = CategoryListNotifier(mockService);

      expect(() => notifier.updateCategory(id: 1, name: '新'), throwsException);
