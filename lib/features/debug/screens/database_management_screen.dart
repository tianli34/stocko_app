...
class DatabaseManagementScreen extends ConsumerWidget {
          ...
          actions: [
            TextButton(
              onPressed: () async {
                ...
                await _clearSalesTransactionsData(context, ref);
              },
              style: TextButton.styleFrom(foregroundColor: Colors.red),
              child: const Text('清空'),
            ),
  ...
  Future<void> _clearSalesTransactionsData(
    BuildContext context,
    WidgetRef ref,
  ) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('此操作将清空所有销售交易数据，确定继续吗？'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(false),
            child: const Text('取消'),
    ...
    if (confirmed == true) {
      try {
        final database = ref.read(appDatabaseProvider);
        await database.delete(database.salesTransaction).go();
        
        if (context.mounted) {
          showAppSnackBar(context, message: '✅ 销售交易数据已清空');
        }
      } catch (e) {
        print('清空销售交易数据时出错: $e');
        if (context.mounted) {
                    ...
                    child: ListTile(
                      ...
                      subtitle: Column(
                        ...
                        children: [
                          ...
                          if (stock.averageUnitPriceInSis > 0)
                            Text(
